...
**SDD Gate:** G4
**Design refs:** `SPEC.md §7.4`, `SPEC.md §7.7`, `SPEC.md §14`, `SPEC.md §7.10`, `docs/architecture/ADR-016-secrets-management-openbao.md`, `docs/fintech-ledger-features.md §10`, `docs/user-journeys.md §5`
...
4. Keys from OpenBao (E09-T05); never from disk in prod.
...
2. Argon2id-hashed storage in database; secret shown exactly once by a privileged create
...
**Files:**
- Create: `internal/infrastructure/secrets/openbao/{client.go,loader.go,dynamic_db.go,cache.go}`
**Steps:**
1. OpenBao client using `github.com/hashicorp/vault/api` with Token/AppRole auth and auto-renewal.
2. Dynamic database credentials: lease ephemeral 1-hour `app_user` credentials from OpenBao PostgreSQL engine.
3. `{{ secret:path }}` resolution in E01-T03 loader with background TTL refresh.
4. Implements: `SecretManager` port (contract: E06-T12).
**Acceptance Criteria:**
- [ ] Missing secret fails boot with the secret path in the error (test).
- [ ] Rotation and dynamic lease renewal succeed within TTL without restart (test with fake).
**Story Points:** 2
**Depends On:** E01-T03, E01-T08, E06-T12, E07.1-T01
**Related Docs:** `SPEC.md §7.7`, `docs/architecture/ADR-016-secrets-management-openbao.md`
**SDD Gate:** G4
...
**Files:**
- Create: `internal/infrastructure/crypto/{envelope.go,transit.go,pii.go}`
**Steps:**
1. OpenBao Transit engine adapter: Encrypt/Decrypt/Re-wrap with AAD (tenant+record IDs); master keys never leave OpenBao.
2. PII markers: struct-tag driven (`pii:"email"`) encryption on write, decryption on authorized read.
3. PAN tokenization adapter interface backed by OpenBao Transit engine.
...
**Related Docs:** `SPEC.md §14`, `docs/architecture/ADR-016-secrets-management-openbao.md`, `docs/fintech-ledger-features.md §10`, `tasks/epics/E04-compliance.md#E04-T04`
**SDD Gate:** G4
...
**Story Points:** 3
**Depends On:** E01-T08, E06-T12, E07.1-T01
