...
type TenantStore interface {
	// Create stores a new tenant record. Strong write; fails on duplicate ID or name.
	Create(ctx context.Context, tenant entity.TenantData) error
...
func (s *TenantService) ProvisionTenant(ctx context.Context, req port.ProvisionTenantRequest) (port.TenantResult, error) {
	...
	tenantID := valueobject.TenantID(s.ids.NewID())
	eventID := s.ids.NewID()
	now := s.clock.Now().UTC()
	...
	return s.runTenantCommand(ctx, port.Subject{ID: req.Actor}, "tenant.provision", "platform/tenants", rec,
		func(ctx context.Context, tx port.Tx) (entity.TenantData, *port.OutboxFact, error) {
			...
			plan, err := service.ValidateOnboarding(service.OnboardingRequest{
				TenantID:       tenantID,
				LedgerID:       valueobject.LedgerID(s.ids.NewID()),
				Name:           req.Name,
				Region:         req.Region,
			...
			if err := s.tenants.Create(ctx, plan.Tenant); err != nil {
				return entity.TenantData{}, nil, err
			}
			payload, err := jsonparser.Marshal(plan.Event)
			if err != nil {
				return entity.TenantData{}, nil, err
			}
			return plan.Tenant, &port.OutboxFact{
				TenantID:    plan.Tenant.ID,
				EventType:   "tenant.created.v1",
				AggregateID: string(plan.Tenant.ID),
				Payload:     payload,
				OccurredAt:  now,
			}, nil
		})
